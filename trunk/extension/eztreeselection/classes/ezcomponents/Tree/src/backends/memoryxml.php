<?php
/**
 * File containing the ezcTreeMemoryXml class.
 *
...
/**
 * ezcTreeMemoryXml is an implementation of a tree backend that operates on
 * an XML string, loaded as member variable and then manipulated in memory.
 *
 * Example:
 * <code>
 * <?php
 *     $xmlString = '<ezselection>
 * <options>
 *	 <option id="0" name="first choice"/>
 *	 <option id="1" name="second choice"/>
 * </options>
 * </ezselection>';
 *     
 *     // Create a new tree in a new XML file
 *     $tree = ezcTreeMemoryXml::create(
 *         $xmlString, 
 *         new ezcTreeXmlInternalDataStore()
 *     );
 * 
 *     // Open an existing XML file containing a tree structure
 *     $tree = new ezcTreeMemoryXml(  
 *         $xmlString,
 *         new ezcTreeXmlInternalDataStore()
 *     );
 * ?>
 ...
 * @mainclass
 */
class ezcTreeMemoryXml extends ezcTreeXml
{
    /**
...
<grammar xmlns:etd="http://components.ez.no/Tree/data" ns="http://components.ez.no/Tree" xmlns="http://relaxng.org/ns/structure/1.0" datatypeLibrary="http://www.w3.org/2001/XMLSchema-datatypes">
  ...
  <define name="node">
    ...
    /**
     * The string that contains the tree as XML string.
     *
     ...
     */
    protected $autoNodeId = 0;
    
    /**
     * Constructs a new ezcTreeMemoryXml object from the XML data $xmlString and using
     * the $store to retrieve data from.
     *
     * @param string $xmlFile
     * @param ezcTreeXmlDataStore $store
     */
    public function __construct( $xmlString, ezcTreeXmlDataStore $store )
    {
        ...
        if ( count( $errors ) )
        {
            // @TODO : cerate a new exception for this. The one used below takes a filename as parameter. Does not apply here
            throw new ezcTreeInvalidXmlException( '<in-memory XML>', $errors );
        }
        ...
        if ( !$valid )
        {
            ...
            libxml_clear_errors();
            // @TODO : cerate a new exception for this. The one used below takes a filename as parameter. Does not apply here            
            throw new ezcTreeInvalidXmlFormatException( '<in-memory XML>', $errors );
        }
    ...
    /**
     * Creates a new XML tree in the string $xmlString using $store as data store.
     *
     ...
     * @param ezcTreeXmlDataStore $store
     * @param string $prefix
     * @return ezcTreeMemoryXml
     */
    public static function create( $xmlString, ezcTreeXmlDataStore $store, $prefix = 'id' )
    {
        ...
        $element->setAttributeNode( new DOMAttr( 'lastNodeId', 0 ) );
            
        $dom->appendChild( $element );

        $tmpXmlString = $dom->saveXML();
        return new ezcTreeMemoryXml( $tmpXmlString, $store, $prefix );
    }

    /**
     * Store the XML as member variable
     */    
    public function saveFile()
    {
        $this->xmlString = $this->dom->saveXML();        
    }    
    
    /**
     * Alias for saveFile(). Awaits redesign.
     */
     ...
     * @return array(string)
     */
    protected function fetchChildIds( $nodeId )
    {
        $childNodes = array();
     ...
     * @param string          $nodeId
     */
    protected function addChildNodesDepthFirst( ezcTreeNodeList $list, $nodeId )
    {
        $className = $this->properties['nodeClassName'];
        foreach ( $this->fetchChildIds( $nodeId ) as $childId )
        {
     ...
     * Returns whether the node with ID $nodeId exists.
     *
     * @return string the value of the member variable storing the Xml string.
     */
    public function getXmlString()
    {
        // @TODO : Throw a proper exception here in case $this->xmlString is empty.
        //         Cannot be done currently, eZ Publish not being Exception friendly 
        //         ( and this new back end was originally made in the scope of an eZ Publish project )
